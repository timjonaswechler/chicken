use bevy::{
    log::{error, info, warn},
    prelude::{App, Event, On, Plugin, Res, ResMut, Resource, Time, Timer, TimerMode, Update},
};

#[cfg(feature = "debug")]
use bevy_egui::{egui, EguiContexts, EguiPrimaryContextPass};

pub struct ChickenNotificationPlugin;

impl Plugin for ChickenNotificationPlugin {
    fn build(&self, app: &mut App) {
        ...
        #[cfg(feature = "debug")]
        {
            app.add_systems(EguiPrimaryContextPass, ui_notification_system);
        }
    }
}
            ...
            for note in &mut queue.messages {
                let (bg_color, stroke_color) = match note.type_ {
                    NotificationType::Info => (
                        egui::Color32::from_rgb(20, 20, 50),
                        egui::Color32::LIGHT_BLUE,
                    ),
                    NotificationType::Success => {
                        (egui::Color32::from_rgb(20, 50, 20), egui::Color32::GREEN)
                    }
                    NotificationType::Warning => {
                        (egui::Color32::from_rgb(50, 50, 20), egui::Color32::YELLOW)
                    }
                    NotificationType::Error => {
                        (egui::Color32::from_rgb(50, 20, 20), egui::Color32::RED)
                    }
                };
